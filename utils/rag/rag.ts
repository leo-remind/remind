...
export async function chat(db: SQLiteDatabase, query: string): Promise<Object> {
  ...
  console.log("[retrieve-relevant] yaya");
  let conversations = await retrieveRelevantConversations(db, queryEmbeddings);
  //let peopleIds = getRelevantPeople(db, conversations);
  console.log("[relevant-retrieved] retrieve complete");
  // console.log(conversations);

  let relPhotos = await retrieveRelevantImages(db, queryEmbeddings);

  // if (relPhotos.length == 0) {
  //   return {
  //     answer: "No relevant photos found",
  //     photos: [],
  //     people: [],
  //   }
  // }
  if (conversations.length == 0) {
    return {
      ...
      photos: (relPhotos.length == 0) ? [] : relPhotos,
      people: [],
    }
  }
...
async function retrieveRelevantImages(
  db: SQLiteDatabase,
  queryEmbedding: Float32Array
): Promise<Array<VectorResponse>> {
  ...
  let sortedRows = [];
    ...
    for (const row of allRows) {
      try {
        const buffer = await row.caption_vector.arrayBuffer();
  ...
  return sortedRows
    ...
    .filter((num) => num.similarity);
}

async function retrieveRelevantConversations(
  db: SQLiteDatabase,
  queryEmbedding: Float32Array
): Promise<Array<VectorResponse>> {
  ...
  console.log("[retrieve-relevant] retrieved rows");
  ...
  try {
    ...
    for (const row of allRows) {
      try {
        const buffer = new Float32Array(eval(row.summary_vector));
        const vector = new Float32Array(buffer);
        const similarity = cosineSimilarity(vector, queryEmbedding);
...
export async function getHighestMatchingFace(
  db: SQLiteDatabase,
  targetFaceEmbedding: Float32Array,
  threshold: number
): Promise<number | null> {
...
async function getRelevantPeople(
  db: SQLiteDatabase,
  conversations: Array<VectorResponse>
): Promise<Array<number>> {
  const conversationIds = conversations.map((conv) => conv.id);

  const peopleFrequency = await db.getAllAsync<{
    personId: number;
    frequency: number;
  }>(`
    ...
    messages: [
      {
        role: "system",
        content:
          "You are a helpful assistant that answers questions based on provided conversation histories. Use the context provided to give accurate and relevant answers. Ensure that your replies are concise and in one paragraph",
      },
      {
        role: "user",
        content: `Context:\n${contextString}\n\nQuestion: ${query}`,
